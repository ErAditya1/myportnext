      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-zinc-900 border border-zinc-800 p-6 rounded-xl">
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <input required type="text" className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="Post title" />
          </div>
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-2">Slug *</label>
            <input required type="text" className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" value={formData.slug} onChange={e => setFormData({...formData, slug: e.target.value})} placeholder="URL-friendly-slug" />
          </div>
          <div>
        ...
        <div>
           ...
           {!previewUrl ? (
             <div className="border-2 border-dashed border-zinc-700 rounded-xl p-8 text-center bg-zinc-950/50 hover:bg-zinc-900 transition relative">
                ...
                <p className="text-zinc-400 text-sm">Upload featured image</p>
             </div>
           ) : (
             <div className="relative w-64 h-40 rounded-lg overflow-hidden border border-zinc-700">
               <img src={previewUrl} alt="Preview" className="object-cover w-full h-full" />
               <button type="button" onClick={removeFile} className="absolute top-2 right-2 bg-black/50 hover:bg-red-500 p-2 rounded-full text-white transition">
                 <X size={16} />
               </button>
             </div>
           )}
        ...
        <div className="pt-8 border-t border-zinc-800 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6">
            <div>
              ...
              <input type="text" className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" value={formData.metaTitle} onChange={e => setFormData({...formData, metaTitle: e.target.value})} placeholder="SEO Title (defaults to Post Title)" />
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-300 mb-2">Meta Description</label>
              <textarea rows={3} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" value={formData.metaDescription} onChange={e => setFormData({...formData, metaDescription: e.target.value})} placeholder="SEO Description (defaults to content excerpt)"></textarea>
            </div>
          </div>
        </div>
